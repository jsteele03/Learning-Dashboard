...
def _safe_yoy(series_id: str) -> Optional[float]:
    ...
    try:
        ...
        print(f"Error fetching YoY for {series_id}: {e}")
        return None
...
def _safe_qoq_annualized(series_id: str) -> Optional[float]:
    ...
    try:
        ...
        print(f"Error fetching QoQ for {series_id}: {e}")
        return None
...
def payroll_employment_change() -> Optional[float]:
    ...
    try:
        ...
        print(f"Error fetching PAYEMS: {e}")
        return None
...
def yield_curve_3m_10y() -> Optional[float]:
    ...
    if fred is None:
        return None
    try:
        # Only fetch the last 30 days to avoid huge series
        end = datetime.today()
        start = end - timedelta(days=30)
        t10y = fred.get_series("DGS10", observation_start=start, observation_end=end).dropna().iloc[-1]
        t3m = fred.get_series("DTB3", observation_start=start, observation_end=end).dropna().iloc[-1]
